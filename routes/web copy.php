...
Route::get('/newAccount', [RegisterController::class, 'index'])
    ->middleware('checklicence')
    ->name('register');
Route::post('/Account/store', [RegisterController::class, 'store'])
    ->middleware('checklicence')
    ->name('user.new');
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])
    ...
    ->group(function () {
        Route::get('/utilisateur', [UserController::class, 'index'])
            ->middleware('checklicence', 'checkrole')
            ->name('user');
    })
    ->group(function () {
    ...
    ->group(function () {
        Route::post('/utilisateur/store', [UserController::class, 'store'])
            ->middleware('checklicence', 'checkrole')
            ->name('user.store');
    })
    ->group(function () {
        Route::post('/utilisateur/store_role', [UserController::class, 'store_role'])
            ->middleware('checklicence', 'checkrole')
            ->name('user.store_role');
    })
    ->group(function () {
        Route::get('/utilisateur/{id}/edit', [UserController::class, 'edit'])
            ->middleware('checklicence', 'checkrole')
            ->name('users.edit');
    })
    ->group(function () {
    ...
    ->group(function () {
        Route::put('/update_profile/{id}', [UserController::class, 'update_profile'])
            ->middleware('checklicence')
    ...
    ->group(function () {
        Route::get('/message/{pivotId}', [MessageController::class, 'show'])
            ->middleware('checklicence')
            ->name('message.show');
    })
    ->group(function () {
        Route::get('/messageSended/{pivotId}', [MessageController::class, 'showSend'])
            ->middleware('checklicence')
            ->name('message.showSend');
    })
    ->group(function () {
        Route::post('/indentification/store/{id}', [ServiceController::class, 'identUser'])
            ->middleware('checklicence')
            ->name('service.ident');
    })
    ->group(function () {
        Route::get('/pdf/{id}', [PdfView::class, 'index'])
            ->middleware('checklicence')
            ->name('pdf.view');
    })
    ->group(function () {
    ...
    ->group(function () {
        Route::get('/export-historque-pdf', [HistoryController::class, 'exportPDF'])
            ->middleware('checklicence')
            ->name('history.export');
    })
    ->group(function () {
...
Route::get('/api/users', function () {
    $users = User::all(['id', 'name']); // On récupère uniquement l'id et le nom
    return response()->json($users);
});


Route::get('/api/folder/{id}', function ($id): JsonResponse {
    $folder = Folder::find($id, ['id', 'name']);
    if (!$folder) {
...
Route::post('/api/folder/{folder}/permissions', function (Request $request, Folder $folder) {
    $permissions = $request->validate([
        'users' => 'required|array',
        'users.*' => 'in:read,write,read_write'
    ]);

    foreach ($permissions['users'] as $userId => $role) {
        $folder->users()->syncWithoutDetaching([$userId => ['permission' => $role]]);
    }
