<div id="restrictUserModal" wire:ignore.self
    ...
    <div class=" rounded-lg shadow-lg p-6 w-full max-w-3xl  max-h-[85vh] overflow-y-auto" style="background-color: rgb(240, 233, 233)">
        <!-- Informations sur le dossier -->
        <div class="mb-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-2xl font-bold text-gray-800">Informations du dossier</h2>
                <button type="button" @click="closeRestrictUserModal"
        ...
        <div class="grid grid-cols md:grid-cols-2">
            <div>
                <h3 class="text-lg font-semibold mb-3">Gérer les restrictions</h3>
            </div>
            <div class="relative">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full  text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody id="usersPermissionsList" class="divide-y divide-gray-200 bg-white">
                    <!-- Bouton pour attribuer les restrictions à tous les utilisateurs du service -->
                    <tr>
                        <td>Appliquer à tous les utilisateurs du service</td>
                        <td class="px-4 py-2 text-right">
                            <div class="flex space-x-2">
                                <button wire:click="assignRestrictionsToService('0')"
                                        class="px-2 py-1 text-xs bg-yellow-500 hover:bg-yellow-600 text-white rounded">
                                    Non
                                </button>
                                <button wire:click="assignRestrictionsToService('1')"
                                        class="px-2 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded">
                                    Oui
                                </button>
                                <span wire:loading wire:target="assignRestrictionsToService" role="status">
                                    <svg aria-hidden="true"
                                        class="inline w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                                        viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
                    ...
                    @if ($allUsers)
                        @foreach ($allUsers as $user)
                            <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                                ...
                                <td class="px-4 py-2 text-right">
                                    <form method="POST" wire:submit.prevent="saveRestriction({{ $user->id }})">
                                        @csrf

                                        <div class="inline-flex items-center space-x-4 mr-4">
                                            <label class="inline-flex items-center">
                                                <input type="radio" wire:model="restrictions.{{ $user->id }}"
                                                    value="0" class="form-radio h-4 w-4 text-blue-600">
                                                <span class="ml-2">Non</span>
                                            </label>

                                            <label class="inline-flex items-center">
                                                <input type="radio" wire:model="restrictions.{{ $user->id }}"
                                                    value="1" class="form-radio h-4 w-4 text-blue-600">
                                        ...
                                        <button type="submit"
                                            ...
                                            Enregistrer
                                            <span wire:loading wire:target="saveRestriction({{ $user->id }})"
                                                role="status">
                                                <svg aria-hidden="true"
                                                    class="inline w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
