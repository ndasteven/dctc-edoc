                ...
                <aside
                    ...
                    <div class="pdf-viewer-container">
                        ...
                        @if (session()->has('error'))
                            <div
                                class="alert alert-error mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                        ...
                        @if (!$hasAccess)
                            <div class="no-access-message text-center py-8">
                        ...
                        @else
                            {{-- Contenu du document (PDF viewer) --}}
                            <div class="pdf-content mb-6">
                                {{-- Ici vous pouvez intégrer votre visualiseur PDF --}}
                                <div
                            ...
                            <div class="actions-buttons">
                                <div class="flex flex-col items-center space-y-2 w-full">
                                    ...
                                    {{-- Bouton "Laisser un message" - Visible pour L et LE --}}
                                    ...
                                    {{-- Bouton "Ouvrir hors de l'application" - Visible pour L, E et LE --}}
                                    @if ($showOpenButton)
                                        <a href="{{ asset('storage/' . $document->filename) }}" target="_blank">
                                            <button type="button"
                                                class="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-gray-700 rounded-lg hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    ...
                                    {{-- Bouton "Éditer" - Visible pour E et LE --}}
                                    @if ($showEditButton)
                                        <a href="{{ route('documents.edit', $document->id) }}" target="_blank">
                                            <button type="button"
                                                class="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-green-500 rounded-lg hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                ...
                                {{-- Message si aucun bouton n'est disponible --}}
...
<script>
    // Injecter la variable PHP dans JavaScript
    const isReadOnlyMode = @json($permission === 'L');
    
    // Script de protection pour PDF en mode lecture seule
    (function() {
        'use strict';
        
        if (!isReadOnlyMode) {
            return; // Ne pas appliquer les restrictions si ce n'est pas en mode lecture
        }

        // 1. Désactiver le clic droit sur toute la page
        ...
        // 2. Désactiver les raccourcis clavier dangereux
        document.addEventListener('keydown', function(e) {
            // Désactiver Ctrl+S (Enregistrer)
            ...
            // Désactiver Ctrl+A (Sélectionner tout)
        ...
        // 3. Masquer les boutons de contrôle PDF
        function hidePDFControls() {
            const style = document.createElement('style');
            style.textContent = `
                /* Masquer la barre d'outils du PDF */
                iframe {
                    pointer-events: auto !important;
                }
                
                /* Styles pour masquer les contrôles PDF dans l'iframe */
                ...
                /* Masquer les boutons de contrôle potentiels */
                .pdf-controls,
                .pdf-toolbar,
                .download-btn,
                .print-btn {
                ...
                /* Masquer le curseur de texte sur le PDF */
        ...
        // 4. Surveiller les iframes PDF et appliquer les restrictions
        function monitorPDFIframes() {
            ...
            iframes.forEach(iframe => {
                ...
                if (iframe.src && (iframe.src.includes('.pdf') || iframe.src.includes('pdf'))) {
                    ...
                    // Ajouter un overlay invisible pour bloquer les interactions
                    ...
                    // Ajouter les gestionnaires d'événements à l'overlay
                    ...
                    overlay.addEventListener('mousedown', function(e) {
                        // Bloquer le clic droit et le clic molette
                    ...
                    // Positionner l'iframe relativement pour l'overlay
                    ...
                    // Masquer la barre d'outils du PDF (si possible)
                    iframe.addEventListener('load', function() {
                        try {
                            // Tenter de masquer les contrôles du PDF
                            const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                            if (iframeDoc) {
                                const toolbars = iframeDoc.querySelectorAll('[class*="toolbar"], [id*="toolbar"], [class*="controls"], [id*="controls"]');
                                toolbars.forEach(toolbar => {
                                    toolbar.style.display = 'none';
                                });
                            }
                        } catch (e) {
                            // Les restrictions CORS peuvent empêcher l'accès au contenu de l'iframe
                            console.log('Impossible d\'accéder au contenu de l\'iframe PDF');
                        }
                    });
                }
            });
        ...
        // 5. Bloquer l'ouverture de nouveaux onglets/fenêtres
        window.addEventListener('beforeunload', function(e) {
            // Optionnel: avertir avant de quitter la page
            // e.preventDefault();
            // e.returnValue = '';
        });

        // 6. Désactiver la fonction d'impression de la fenêtre
        window.print = function() {
            showWarning('L\'impression n\'est pas autorisée pour ce document.');
            return false;
        };

        // 7. Message d'avertissement pour les tentatives de contournement
        function showWarning(message = 'Action non autorisée') {
            ...
            `;
            warning.innerHTML = `
                <h3>⚠️ Action non autorisée</h3>
                <p>${message}</p>
                <p><small>Ce document est en mode lecture seule.</small></p>
            `;
        ...
        // 8. Surveiller les tentatives de contournement
        let warningCount = 0;
        document.addEventListener('keydown', function(e) {
            ...
            const isRestricted = restrictedCombos.some(combo => combo);
            
            if (isRestricted) {
                warningCount++;
                if (warningCount <= 3) {
                    showWarning('Les raccourcis clavier sont désactivés en mode lecture.');
                }
            }
        });

        // 9. Protection supplémentaire pour empêcher la manipulation du DOM
        Object.defineProperty(document, 'oncontextmenu', {
            value: function() { return false; },
            writable: false
        });

        // 10. Désactiver la sélection de texte globale
        document.addEventListener('selectstart', function(e) {
            e.preventDefault();
            return false;
        });

        // 11. Désactiver le glisser-déposer
        document.addEventListener('dragstart', function(e) {
            e.preventDefault();
            return false;
        });

        // 12. Bloquer les tentatives d'ouverture dans un nouvel onglet
        document.addEventListener('click', function(e) {
            if (e.ctrlKey || e.metaKey || e.button === 1) {
                e.preventDefault();
                e.stopPropagation();
                showWarning('L\'ouverture dans un nouvel onglet n\'est pas autorisée.');
                return false;
            }
        });
        ...
        function initializeProtections() {
            ...
            // Observer les changements DOM pour les nouveaux iframes
