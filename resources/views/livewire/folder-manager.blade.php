...
<div class="mx-auto" >
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-8 relative" >
            ...
            @if (strlen($SessionServiceinfo)>0)
              <small class="breadcrumb inline-flex items-center">
                    ...
                    <span class="inline-flex items-center">
                        ...
                        <a 
                            href="#" 
                            wire:click.prevent="navigateToFolder({{ $folder['id'] ?? 'null' }})"
        ...
        <div class="flex justify-end p-3">
            <button data-modal-target="uploadFile" data-modal-toggle="uploadFile" type="button" wire:click="infoIdFocus"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-white relative overflow-auto" style="height: 60vh ">      
        @foreach($folders as $index=>$folder)
            <div wire:key={{ $index }} class="p-2 border-0 rounded iconButton  grid relative click-right w-40 h-32 rounded shadow-md overflow-hidden bg-cover bg-center group hover:scale-105 transition" style="background-image: url({{ asset('img/folder.png') }});" data-dropdown-id="dropdownRight-{{ $folder->id }}" data-dropdown-placement="right">
                ...
                <div class="flex justify-end">
                    ...
                    @endif
                    <button class="text-white hover:text-gray-300 trois-point"><span wire:loading wire:target="deleteFolder">
                        <span role="status">
                            <svg aria-hidden="true" class="w-4 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                                <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                ...
                <a href="{{ route('folders.show', $folder->id) }}" class="text-blue-600 font-semibold">
                  <!-- Nom du dossier (centré ou en bas) -->
                    <div class="text-gray-600 text-sm font-semibold text-center">
                        @if( \Illuminate\Support\Str::length($folder->name)>30) {{ \Illuminate\Support\Str::limit($folder->name, 35) }} @else {{ $folder->name }} @endif
                    </div>
                </a>
                ...
                <div class="flex justify-between items-center mt-1">
                  ...
                  <!-- Dropdown menu -->
                <div  id="dropdownRight-{{ $folder->id }}" class="z-10 absolute hidden bg-blue-600 divide-y divide-gray-100  shadow-sm w-44 dark:bg-gray-700">
                    <ul class=" text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownRightButton">
                    <li style="border-top: solid 1px white">
                        <small>
                            <button @if ($folder->verrouille) disabled @endif href="#" class="block px-4 py-1 text-white hover:bg-gray-600 dark:hover:bg-gray-600 dark:hover:text-white hover:text-white inline-flex flex justify-between items-center w-full" @click='clickeditfolder'   wire:click="getFolderId({{ $folder->id }})"><span>Renommer</span> 
                            <svg class="w-4 h-4 text-whitedark:text-white hover:text-white " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    ...
                    <li style="border-top: solid 1px white">
                        <small>
                            <button @if ($folder->verrouille) disabled @endif href="#" class="block px-4 py-1 text-white hover:bg-gray-600 hover:text-white  dark:hover:bg-gray-600 dark:hover:text-white inline-flex flex justify-between items-center w-full " @click="clickModal"  wire:click="getFolderId({{ $folder->id }})"><span>Ajouter fichier</span><span></span>
                    ...
                    <small>
                        <li style="border-top: solid 1px white">
                        <button href="#" class="block px-4 text-white  py-1 hover:bg-gray-600 hover:text-white  dark:hover:bg-gray-600 dark:hover:text-white inline-flex flex justify-between items-center w-full " @click="clickModalPropriete"  wire:click="getIds({{ $folder->id}},'folder')"><span>propriété</span><span></span>
                            <svg class="w-4 h-4 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9V4a1 1 0 0 0-1-1H8.914a1 1 0 0 0-.707.293L4.293 7.207A1 1 0 0 0 4 7.914V20a1 1 0 0 0 1 1h4M9 3v4a1 1 0 0 1-1 1H4m11 6v4m-2-2h4m3 0a5 5 0 1 1-10 0 5 5 0 0 1 10 0Z"/>
                            </svg>  
                        </button>
    ...
    <!-- Bouton menu (en haut à droite) -->
            ...
            <div class="text-gray-600 text-sm font-semibold text-center bg-white px-4">
                <small class="text-xs">@if( \Illuminate\Support\Str::length($file->nom)>30) {{ \Illuminate\Support\Str::limit($file->nom, 30) }}.{{ $type }} @else {{ $file->nom }} @endif </small>   
            </div>
            
                <div class="flex  justify-center">
                @if ($file->type=="pdf")
                  <svg class=" text-red-600 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" viewBox="0 0 24 24">
                ...
                @elseif ($type=="docx" or $type=="doc")
                    <svg class="w-12 h-12 text-blue-200 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                ...
                @elseif ($type=="xls" or $type=="xlsx")
                    <svg class="w-12 h-12 text-green-600 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M6 16v-3h.375a.626.626 0 0 1 .625.626v1.749a.626.626 0 0 1-.626.625H6Zm6-2.5a.5.5 0 1 1 1 0v2a.5.5 0 0 1-1 0v-2Z"/>
                        <path fill-rule="evenodd" d="M11 7V2h7a2 2 0 0 1 2 2v5h1a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1h-1a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2H3a1 1 0 0 1-1-1v-9a1 1 0 0 1 1-1h6a2 2 0 0 0 2-2Zm7.683 6.006 1.335-.024-.037-2-1.327.024a2.647 2.647 0 0 0-2.636 2.647v1.706a2.647 2.647 0 0 0 2.647 2.647H20v-2h-1.335a.647.647 0 0 1-.647-.647v-1.706a.647.647 0 0 1 .647-.647h.018ZM5 11a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1h1.376A2.626 2.626 0 0 0 9 15.375v-1.75A2.626 2.626 0 0 0 6.375 11H5Zm7.5 0a2.5 2.5 0 0 0-2.5 2.5v2a2.5 2.5 0 0 0 5 0v-2a2.5 2.5 0 0 0-2.5-2.5Z" clip-rule="evenodd"/>
                        <path d="M9 7V2.221a2 2 0 0 0-.5.365L4.586 6.5a2 2 0 0 0-.365.5H9Z"/>
                    </svg>                                     
                @elseif ($type=="csv")
                <svg class="w-12 h-12 text-green-400 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div id="dropdownRight-{{ $file->id }}" class="z-10 absolute top-0 hidden  bg-blue-600 divide-y divide-gray-100  shadow-sm w-40 dark:bg-gray-700">
                <ul class=" text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownRightButton" id="ul">
                    <li class="">
                        <small>
                            <button @if ($file->verrouille) disabled @endif href="#" class="block px-4 py-1 text-white hover:bg-gray-600 hover:text-black  dark:hover:bg-gray-600 dark:hover:text-white inline-flex flex justify-between items-center w-full " @click='clickeditFile' wire:click="getFileId({{ $file->id }})"><span>Renommer</span><span wire:loading wire:target='getFileId'>
                <span role="status">
                    <svg aria-hidden="true" class="w-4 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                                ...
                                <svg class="w-4 h-4 text-white dark:text-white hover:text-white " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z"/>
                                </svg>
                            </button>
                        </small> <small>
                            <li style="border-top: solid 1px white">
                             <button href="#" class="block px-4 text-white  py-1 hover:bg-gray-600 hover:text-white  dark:hover:bg-gray-600 dark:hover:text-white inline-flex flex justify-between items-center w-full " @click="clickModalPropriete"  wire:click="getIds({{ $file->id}},'file')"><span>propriété</span><span></span>
                                <svg class="w-4 h-4 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9V4a1 1 0 0 0-1-1H8.914a1 1 0 0 0-.707.293L4.293 7.207A1 1 0 0 0 4 7.914V20a1 1 0 0 0 1 1h4M9 3v4a1 1 0 0 1-1 1H4m11 6v4m-2-2h4m3 0a5 5 0 1 1-10 0 5 5 0 0 1 10 0Z"/>
                                </svg>  
                            </button>   
    ...
    <div class=" grid grid-cols-1 flex bg-white justify-center " style="padding: 100px;  ">
        <div style="text-align: center">
            <img class="h-auto max-w-lg mx-auto" src="{{ asset('img/vide.svg') }}" style="height: 100px" alt="image description">
            vide
        </div>  
...
<script>
   ...
   document.addEventListener('livewire:initialized', () => {
    ...
    function clickdroit(){
        ...
        document.querySelectorAll('.trois-point').forEach(function(el) {
            el.addEventListener('click', (event) => {
                ...
                const dropdownId = el.closest('[data-dropdown-id]').getAttribute('data-dropdown-id');
    ...
    const lien_id = document.querySelectorAll('#ul')
        lien_id.forEach((el)=>{
            el.addEventListener('click', (e)=>{ // pour desactiver la redirection vers les affichage du fichier
            e.preventDefault()
        })
        })
}
...
document.addEventListener('changeUrl', function (event) {//permet d'ecouter evenement changeUrl de son controller et  naviger entre les folder sans rafraichir la page
