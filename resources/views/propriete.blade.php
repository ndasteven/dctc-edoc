...
<div id="propiete" tabindex="-1" aria-hidden="true"
    ...
    <div 
        ...
        x-transition
        x-init="
            window.addEventListener('successVerrou', () => {
                show = true;
    ...
    >
        <div class="max-w-sm">
            <div class="p-4 mb-4 text-sm text-white bg-green-500 dark:bg-green-800 dark:text-white" role="alert" >
                <span class="font-medium">Code accepté!</span> .
            </div>
        </div>
    </div>
    <div 
        ...
        x-transition
        x-init="
            window.addEventListener('errorVerrou', () => {
                show = true;
                setTimeout(() => show = false, 3000);
            });
        "
    ...
    >
        <div class="max-w-sm">
            <div class="p-4 mb-4 text-sm text-white bg-red-500 dark:bg-green-800 dark:text-white" role="alert" >
                <span class="font-medium">Erreur de code!</span> .
            </div>
        </div>
    </div>
            ...
            <div class="p-6 grid md:grid-cols-3 gap-3 text-white">
                ...
                <div class="space-y-6 grid col-span-1  flex item-center">
                    ...
                    <div class=" p-4 rounded-lg shadow-md space-y-2 text-sm">
                         @if ($infoPropriete)
                        <div>Nom :<strong>{{ $infoPropriete->name ?? $infoPropriete->nom ?? 'pas de nom'  }}</strong> </div>
                        <div>Auteur : <strong>{{ $infoPropriete->user->name ?? 'Utilisateur supprimé' }}</strong> </div>
                        <div>Date de création: <strong> {{ $infoPropriete->created_at->format('d-m-Y à H:i:s') }}</strong></div>
                        @else
